...
| Supervisor | Candidate identity, target role, milestones and supporting evidence for direct-report candidates only; the confidential candidate fields (`performance_score`, `potential_score`, `nine_box_label`, `readiness_level`, `mentor_id`, `status`) and the confidential position fields (`vacancy_risk`, `risk_factors`, `estimated_vacancy_date`) are blanked | Direct-report development milestones only |
