...
class CompetencyFrameworkSeeder extends Seeder
{
     ...
     *
     * Two reviews are seeded and both carry the same created_at, so the target
     * is picked by status rather than by order: `finished` is the one the
     * reviewer has already worked through, so it has scores to show. The `draft`
     * review is deliberately left with empty KPI rows — that is what the
     * supervisor login opens to fill in.
     */
    private function seedReviewScores(): void
    {
        $review = DB::table('performance_reviews')->where('status', 'finished')->first();
        ...
        $scores = [
            'Patient Satisfaction Score' => 4.25,
            'Hand Hygiene Audit Score' => 4.50,
            'Medication Error Rate' => 4.50,
            'Mandatory Training Completion' => 3.25,
            'Supervision & Mentoring' => 4.50,
            'Roster & Skill-Mix Compliance' => 3.00,
            'Handover Quality (SBAR)' => 4.25,
        ];

        foreach ($scores as $kpiName => $supervisor) {
            ...
            DB::table('review_kpi_scores')->insert([
                ...
                // A straight copy of the reviewer's score, which is what the
                // controller stores now there is only one voice to blend.
        ...
        DB::table('performance_reviews')->where('review_id', $review->review_id)->update([
            ...
            'status' => 'finished',
            // A finished review carries the reviewer's sign-off, the same way
            // saveScores() stamps one, so the seeded row is not a state the app
            // itself could never produce.
            'signed_at' => now(),
            'digital_signature' => hash('sha256', $review->review_id.'|'.$review->reviewer_id.'|'.now()->toIso8601String()),
            'strengths_text' => 'Consistently strong clinical judgement and calm leadership during high-acuity periods. Handover quality is reliably high.',
        ...
        $this->seedUnscoredReview();
    }

    /**
     * Attach KPI rows — with no scores on them — to the review that is still a
     * draft.
     *
     * A review with no KPIs attached has nothing to rate, so the supervisor
     * login would open the scoring screen to an empty table and the draft path
     * could not be walked at all. Rows without values are exactly the state a
     * reviewer leaves behind after attaching KPIs and before scoring them.
     */
    private function seedUnscoredReview(): void
    {
        $review = DB::table('performance_reviews')->where('status', 'draft')->first();
        ...
        $kpis = DB::table('kpi_library')
            ->whereIn('kpi_name', [
                ...
                'Handover Quality (SBAR)',
            ])
            ->pluck('kpi_id');

        foreach ($kpis as $kpiId) {
            DB::table('review_kpi_scores')->insert([
                'score_id' => (string) Str::uuid(),
                'review_id' => $review->review_id,
                'kpi_id' => $kpiId,
